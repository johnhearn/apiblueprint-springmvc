(function() {
    ...
    function renderFiles(api, options) {
    	...
    	var templates = {
    		...
    		'e2etest': fs.readFileSync(path.join(templatesDir, 'end2endtest.mustache'), templateFileEncoding)
    	};
    	var partials = {
			...
			resourceend2end: fs.readFileSync(path.join(templatesDir, 'resourceend2end.mustache'), templateFileEncoding),
		...
		var functions = {
			...
			snakeCase: function() {
				return function(text, render) { return snakeCase(render(text)); };
			},
			maptype: function() {
				return function(text, render) {
    	...
    	var end2endTestPath = path.join(options.outputPath, 'controller');
    	...
    	for (var idx = 0; idx < api.resourceGroups.length; idx++) {
    		...
    		data = mustache.render(templates.e2etest, viewModel, partials);
    		promiseWrite(path.join(end2endTestPath, viewModel.controllerClassName + 'IT.java'), data);
    	}

    	// Models - one file for each defined type.
    	var modelsPath = path.join(options.outputPath, 'dto');
    	mkdirIgnoreExists(modelsPath);

        var modelClassDefinitions = parseModels(api, options.skipModelNames, options.flattenParentClasses);
    	console.log('Found ' + modelClassDefinitions.length + ' models');
    	for (idx = 0; idx < modelClassDefinitions.length; idx++) {
    		...
    		viewModel = extend({
    			...
    			modelClassName: pascalCase(modelClassDefinitions[idx].name),
    			modelSubClassName: pascalCase(modelClassDefinitions[idx].subClass),

                extraImports: options.extraImports
    		}, modelClassDefinitions[idx]);
